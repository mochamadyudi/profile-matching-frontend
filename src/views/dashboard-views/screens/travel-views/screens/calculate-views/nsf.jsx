...
function Nsf(props = {})  {
	...
	useEffect(()=> {
		...
		if(Array.isArray(tmpData) && tmpData.length > 0 ){
			if(Array.isArray(tmpData) && tmpData.length > 0){
				...
				newData.map((child)=> {
					let filtered = child.data.filter((child)=> child?.coreFactor === false) ?? []
					let newFiltered = []
					...
					if(Array.isArray(newFiltered) && newFiltered.length > 0 ){
						...
						let value = 0.40 * (reduced.reduce((a,b)=> a + b) ?? 0) / (filtered.length  ?? 0) ?? 0
						
						let calculate = {
							totalValue:value.toFixed(2),
							value: value.toFixed(2),
							totalItem: filtered.length ?? 0,
							description: `(0.40 * ${reduced.reduce((a,b)=> a + b) ?? 0}/${filtered.length ?? 0}) = ${value.toFixed(2)}`
						}
	...
	return (
		<Card bordered={false} title={'Hasil Secondary Factor (NSF)'} loading={loading}>
			<Table
				bordered
				pagination={false}
				rowClassName={(_)=> {
